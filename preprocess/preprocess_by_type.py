...
def df2epochs(big_raw_df, file_type="PCAP", split_type="fixed_size", n_instances=10, eps=1e-5):
    ...
    if split_type == "fixed_size":
        ...
        epoch_time = (big_raw_df[time_col_name].max() - big_raw_df[time_col_name].min()) / n_instances
...
def split_per_epoch(args, fields, df_per_epoch, embed_model, global_max_flow_len, num_epochs, epoch_id, flowkeys_epochidx=None, encode_IP=False):
    ...
    if args.file_type == "PCAP":
        if args.pcap_interarrival == True:
            ...
            fields["interarrival_within_flow"].min_x = float(min(interarrival_within_flow_list))
            fields["interarrival_within_flow"].max_x = float(max(interarrival_within_flow_list))
        else:
    ...
    for group_name, df_group in tqdm(gk):
        ...
        for row_index, row in df_group.iterrows():
            if file_type == "PCAP":
                ...
                if file_type == "CIDDS" or file_type == "TON":
...
def main(args):
    ...
    if args.file_type == "UGR16" or args.file_type == "CIDDS" or args.file_type == "TON":
        ...
        if args.file_type == "CIDDS" or args.file_type == "TON":
    ...
    df_epochs, _tmp_sizeortime = df2epochs(df, file_type=args.file_type,split_type=args.df2epochs, n_instances=args.n_instances)
    ...
    for epoch_id, flowkeys in flow_epochid_keys.items():
        # print("processing epoch {}/{}, # of flows: {}".format(epoch_id+1, args.n_instances, len(flowkeys)))
    ...
    for epoch_id, df_epoch in tqdm(enumerate(df_epochs)):
        ...
        data_out_dir = os.path.join(args.src_dir, "split-{},epochid-{},maxFlowLen-{},Norm-{},vecSize-{},df2epochs-{},interarrival-{},fullIPHdr-{},encodeIP-{}".format(args.split_name, epoch_id, global_max_flow_len, norm_opt_str, args.word2vec_vecSize, args.df2epochs, args.pcap_interarrival | args.netflow_interarrival, args.full_IP_header, args.encode_IP))
