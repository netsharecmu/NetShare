...
class DoppelGANger(object):
    def __init__(self, sess, checkpoint_dir, pretrain_dir, sample_dir, time_path,
                 ...
                 sn_mode=None,
                 # DEBUG FLAGS
                 debug=False, combined_disc=True):
        ...
        if attr_discriminator == None and combined_disc == False:
        ...
        data_attribute_single, data_feature_single, data_gen_flag_single = self.dataset.sample_batch(1)
    ...
    def check_data(self):
        ...
        data_attribute, data_feature, data_gen_flag = self.dataset.sample_batch(self.batch_size)
    ...
    def build_connection(self):
        ...
        self.real_feature_pl = tf.concat(
            ...
            axis=1)
        if self.fix_feature_network:
            self.real_feature_pl = tf.zeros_like(
                self.real_feature_pl)
        self.real_attribute_pl = tf.concat(
            self.real_attribute_pl_l,
            axis=1)
        if self.fix_feature_network:
            self.real_attribute_pl *= tf.tile(self.real_attribute_mask_tensor, [1, self.num_packing])
        ...
        if self.attr_discriminator is not None:
            self.attr_d_real_train_tf, _, self.attr_d_sigmas = self.attr_discriminator.build(
                self.real_attribute_pl,
                train=True,
                sn_op=NO_OPS)
    ...
    def build_loss(self):
        ...
        # hinge loss
            ...
            self.d_loss_fake = tf.reduce_mean(
                tf.nn.relu(1. + self.d_fake_train_tf)
            )
            self.d_loss_fake_unflattened = tf.nn.relu(1. + self.d_fake_train_tf)
            self.d_loss_real = tf.reduce_mean(
                tf.nn.relu(1. - self.d_real_train_tf)
            )
            self.d_loss_real_unflattened = tf.nn.relu(1. - self.d_real_train_tf)
        ...
        if self.attr_discriminator is not None:
            ...
            # hinge loss
                ...
                self.attr_d_loss_fake = tf.reduce_mean(
                    tf.nn.relu(1. + self.attr_d_fake_train_tf)
                )
                self.attr_d_loss_fake_unflattened = tf.nn.relu(1. + self.attr_d_fake_train_tf)
                self.attr_d_loss_real = tf.reduce_mean(
                    tf.nn.relu(1. - self.attr_d_real_train_tf)
                )
                self.attr_d_loss_real_unflattened = tf.nn.relu(1. - self.attr_d_real_train_tf)
            ...
            self.attr_d_interpolates_input_attribute_build_output, \
                self.attr_d_interpolates_input_attribute_build_layers, \
                    _ = self.attr_discriminator.build(
                    interpolates_input_attribute,
                    train=True,
                    sn_op=NO_OPS)
        ...
        if self.attr_discriminator is not None:
            ...
            for i in range(len(self.attr_d_sigmas)):
                if isinstance(self.attr_d_sigmas[i], list):
                    ...
                    self.attr_d_summary.append(tf.summary.scalar(
                        "attr_d/sigma{}".format(i),
                        self.attr_d_sigmas[i]))
    ...
    def train(self, feature_network_checkpoint_path=None, restore=False):
        ...
        for batch_id in tqdm(range(self.iteration)):
            ...
            if global_id > restore_global_id:
                if ((batch_id + 1) % self.epoch_checkpoint_freq == 0 or
                        batch_id == self.iteration - 1):
            ...
            feed_dict = {}
            for i in range(self.num_packing):
                # batch_data_id = batch_data_id_[:, i]
                # batch_data_feature = self.data_feature[batch_data_id]
                # batch_data_attribute = self.data_attribute[batch_data_id]

                batch_data_attribute, batch_data_feature, batch_data_gen_flag = self.dataset.sample_batch(self.batch_size)
