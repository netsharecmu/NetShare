...
def trainDG(config):
    ...
    given_data_attribute = np.load(os.path.join(config["eval_root_folder"], "attr_clean", "chunk_id-{}.npz".format(config["chunk_id"])))["data_attribute"]
    ...
    config["generate_num_train_sample"] = np.shape(given_data_attribute)[0]
    ...
    with tf.Session(config=run_config) as sess:
        ...
        for iteration_id in iteration_range:
            ...
            for generated_samples_idx in range(generatedSamples_per_epoch):
                ...
                print("Number of packet this chunk:", np.sum(gen_flags))
                ...
                os.makedirs(syn_df_folder, exist_ok=True)

                syn_df.to_csv(os.path.join(syn_df_folder, "syn_df_iteration_id-{}.csv".format(iteration_id)), index=False)
