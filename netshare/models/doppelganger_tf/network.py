        ...
        with tf.variable_scope(self.scope_name, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope("layer{}".format(self.num_layers - 1)):
                ...
                # w/ SN
                    ...
                    s_layer, s_sigma = linear(
                        layers[-1], 1, sn_op=sn_op, with_sigma=True, scale=self.scale
                    )
                    layers.append(s_layer)
                    sigmas.append(s_sigma)
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.feature_out_dim = (
            np.sum([t.dim for t in feature_outputs]) * self.sample_len
        )
        ...
        for i in range(len(self.real_attribute_mask) - 1):
            if (
                self.real_attribute_mask[i] == False
                and self.real_attribute_mask[i + 1] == True
            ):
    ...
    def build(
        ...
        with tf.variable_scope(self.scope_name, reuse=tf.AUTO_REUSE):
            ...
            if self.gt_lengths is not None:
                ...
                target_lengths = length_min + (length_max - length_min) * (
                    length_idx_float - tf.to_float(length_idx)
                )
                ...
                if self.use_uniform_lengths:
                    ...
                    target_lengths_feature = tf.to_float(target_lengths) / np.max(
                        self.gt_lengths
                    )
            else:
                target_lengths = None
            ...
            all_attribute = tf.reshape(
                all_attribute, [batch_size, self.attribute_out_dim]
            )
            ...
            with tf.variable_scope("feature", reuse=tf.AUTO_REUSE):
                    ...
                    for j in range(self.sample_len):
                        for k in range(len(self.feature_outputs)):
                            with tf.variable_scope(
                                ...
                                # 1/26/2021 Yucheng: add more layers in the RNN
                                # generator
                ...
                (
                    ...
                    lambda a, b, c, d, e, f, g, h: tf.logical_and(
                        a < time, tf.equal(tf.reduce_max(e), 1)
                    ),
                    compute,
                    (
                        0,
