...
class DoppelGANger(object):
    def __init__(
        ...
        Args:
            ...
            iteration: Number of training iterations (If provided, overiding epoch)
        ...
        # sanity check: must have ONE discriminator
        if attr_discriminator is None and combined_disc == False:
            raise ValueError("There must exist one discriminator at least!")
        ...
        self.combined_disc = combined_disc
        self.debug = debug
        ...
        if dp_noise_multiplier is not None and dp_l2_norm_clip is not None:
            if "tensorflow_privacy.privacy.optimizers.dp_optimizer" not in sys.modules:
                raise RuntimeError(
                    "tensorflow_privacy should be installed for" " DP training"
                )

        self.check_data()

        (
            data_attribute_single,
            data_feature_single,
            data_gen_flag_single,
        ) = self.dataset.sample_batch(1)
        # data_attribute_single, data_feature_single, data_gen_flag_single = self.dataset.sample_batch(self.batch_size)
        # print(self.batch_size)
        # print(data_attribute_single)
        if data_feature_single.shape[1] % self.sample_len != 0:
            raise Exception("length must be a multiple of sample_len")
        self.sample_time = int(data_feature_single.shape[1] / self.sample_len)
        self.sample_feature_dim = data_feature_single.shape[2]
        self.sample_attribute_dim = data_attribute_single.shape[1]
        ...
        # print("\n\n\n\n")
        # print("sample_real_attribute_dim:", self.sample_real_attribute_dim)
        # print("\n\n\n\n\n")
    ...
    def check_data(self):
        ...
        data_attribute, data_feature, data_gen_flag = self.dataset.sample_batch(
            self.batch_size
        )
        ...
        if data_feature.shape[2] != np.sum([t.dim for t in self.data_feature_outputs]):
            raise Exception("feature dimension does not match data_feature_outputs")

        if len(data_gen_flag.shape) != 3:
            raise Exception("Expanded data_gen_flag should be 3 dimension")
    ...
    def build_connection(self):
        ...
        self.real_attribute_mask_tensor = tf.concat(
            self.real_attribute_mask_tensor, axis=1
        )
        ...
        self.g_output_feature_train_tf = tf.concat(
            self.g_output_feature_train_tf_l, axis=1
        )
        self.g_output_attribute_train_tf = tf.concat(
            self.g_output_attribute_train_tf_l, axis=1
        )
        ...
        if self.attr_discriminator is not None:
            self.attr_d_fake_train_tf = self.attr_discriminator.build(
                self.g_output_attribute_train_tf, train=True, sn_op=self.SN_OP
            )[0]
        ...
        for i in range(self.num_packing):
            real_feature_pl = tf.compat.v1.placeholder(
                tf.float32,
                [None, self.sample_time * self.sample_len, self.sample_feature_dim],
        ...
        self.real_feature_pl = tf.concat(self.real_feature_pl_l, axis=1)
        if self.fix_feature_network:
            self.real_feature_pl = tf.zeros_like(self.real_feature_pl)
        self.real_attribute_pl = tf.concat(self.real_attribute_pl_l, axis=1)
        if self.fix_feature_network:
            self.real_attribute_pl *= tf.tile(
                self.real_attribute_mask_tensor, [1, self.num_packing]
            )

        self.d_real_train_tf, _, self.d_sigmas = self.discriminator.build(
            self.real_feature_pl, self.real_attribute_pl, train=True, sn_op=NO_OPS
        )
        self.d_real_test_tf, _, self.d_test_sigmas = self.discriminator.build(
            self.real_feature_pl, self.real_attribute_pl, train=False, sn_op=NO_OPS
        )
        ...
        if self.attr_discriminator is not None:
            (
                ...
                self.real_attribute_pl, train=True, sn_op=NO_OPS
            )
        ...
        self.given_attribute_attribute_pl = tf.compat.v1.placeholder(
            tf.float32, [None, self.sample_real_attribute_dim], name="given_attribute"
        )
        (
        ...
        if get_config("model.should_print_layers", default_value=False):
    ...
    def build_loss(self):
        ...
        if self.attr_discriminator is not None:
            ...
            if self.combined_disc == False:
                self.g_loss = self.g_attr_d_coe * self.g_loss_attr_d

            else:
                self.g_loss = self.g_loss_d + self.g_attr_d_coe * self.g_loss_attr_d
        ...
        # w/ SN
        # hinge loss
        elif self.sn_mode == "SN":
            self.d_loss_fake = tf.reduce_mean(tf.nn.relu(1.0 + self.d_fake_train_tf))
            self.d_loss_fake_unflattened = tf.nn.relu(1.0 + self.d_fake_train_tf)
            self.d_loss_real = tf.reduce_mean(tf.nn.relu(1.0 - self.d_real_train_tf))
            self.d_loss_real_unflattened = tf.nn.relu(1.0 - self.d_real_train_tf)

        else:
            raise ValueError("Unknown SN mode!")

        alpha_dim2 = tf.random.uniform(shape=[batch_size, 1], minval=0.0, maxval=1.0)
        alpha_dim3 = tf.expand_dims(alpha_dim2, 2)
        differences_input_feature = (
            self.g_output_feature_train_tf - self.real_feature_pl
        )

        interpolates_input_feature = (
            self.real_feature_pl + alpha_dim3 * differences_input_feature
        )

        self.d_differences_input_feature = differences_input_feature
        self.d_alpha_dim2 = alpha_dim2
        self.d_alpha_dim3 = alpha_dim3

        differences_input_attribute = (
            self.g_output_attribute_train_tf - self.real_attribute_pl
        )
        interpolates_input_attribute = self.real_attribute_pl + (
            alpha_dim2 * differences_input_attribute
        )

        self.d_interpolates_input_feature = interpolates_input_feature
        self.d_interpolates_input_attribute = interpolates_input_attribute
        ...
        slopes1 = tf.reduce_sum(tf.square(gradients[0]), reduction_indices=[1, 2])
        slopes2 = tf.reduce_sum(tf.square(gradients[1]), reduction_indices=[1])

        self.d_slopes1 = slopes1
        self.d_slopes2 = slopes2

        if self.debug:
            slopes = tf.sqrt(slopes2 + self.EPS)
            self.d_slopes = slopes
        else:
            slopes = tf.sqrt(slopes1 + slopes2 + self.EPS)
            self.d_slopes = slopes

        self.d_loss_gp = tf.reduce_mean((slopes - 1.0) ** 2)
        self.d_loss_gp_unflattened = (slopes - 1.0) ** 2

        self.d_loss = (
            self.d_loss_fake + self.d_loss_real + self.d_gp_coe * self.d_loss_gp
        )

        self.d_loss_unflattened = (
            self.d_loss_fake_unflattened
        ...
        if self.attr_discriminator is not None:
            ...
            # hinge loss
                ...
                self.attr_d_loss_fake = tf.reduce_mean(
                    tf.nn.relu(1.0 + self.attr_d_fake_train_tf)
                )
                self.attr_d_loss_fake_unflattened = tf.nn.relu(
                    1.0 + self.attr_d_fake_train_tf
                )
                self.attr_d_loss_real = tf.reduce_mean(
                    tf.nn.relu(1.0 - self.attr_d_real_train_tf)
                )
                self.attr_d_loss_real_unflattened = tf.nn.relu(
                    1.0 - self.attr_d_real_train_tf
                )
            ...
            self.attr_d_interpolates_input_attribute = interpolates_input_attribute

            (
                self.attr_d_interpolates_input_attribute_build_output,
                self.attr_d_interpolates_input_attribute_build_layers,
                _,
            ) = self.attr_discriminator.build(
                interpolates_input_attribute, train=True, sn_op=NO_OPS
            )
            ...
            slopes1 = tf.reduce_sum(tf.square(gradients[0]), reduction_indices=[1])
            slopes = tf.sqrt(slopes1 + self.EPS)

            self.attr_d_slopes = slopes
            self.attr_d_slopes1 = slopes1
            self.attr_d_tf_square_gradients = gradients

            self.attr_d_loss_gp = tf.reduce_mean((slopes - 1.0) ** 2)
            self.attr_d_loss_gp_unflattened = (slopes - 1.0) ** 2

            self.attr_d_loss = (
                self.attr_d_loss_fake
        ...
        self.g_op = tf.compat.v1.train.AdamOptimizer(self.g_lr, self.g_beta1).minimize(
            self.g_loss, var_list=self.generator.trainable_vars
        )

        if self.dp_noise_multiplier is not None and self.dp_l2_norm_clip is not None:
            print("Using DP optimizer")
            self.d_op = dp_optimizer.DPAdamGaussianOptimizer(
                ...
                self.d_loss_unflattened, var_list=self.discriminator.trainable_vars
            )

        else:
            self.d_op = tf.compat.v1.train.AdamOptimizer(
                self.d_lr, self.d_beta1
            ).minimize(self.d_loss, var_list=self.discriminator.trainable_vars)

        """
        zinan debug
        # train with the same weight for two discs

        discriminator_variables = self.discriminator.trainable_vars
        discriminator_variable_values = self.sess.run(discriminator_variables)
        for var in self.attr_discriminator.trainable_vars:
            var.name -> discriminator_variables
            tf.sess.run(tf.assign(var, value))
        discriminator_variable_values = self.sess.run(self.attr_discriminator.trainable_vars)
        """
        ...
        if self.attr_discriminator is not None:
            if (
                ...
                print("Using DP optimizer")
                self.attr_d_op = dp_optimizer.DPAdamGaussianOptimizer(
                    l2_norm_clip=self.dp_l2_norm_clip,
                    noise_multiplier=self.dp_noise_multiplier,
                    num_microbatches=self.batch_size,
                ...
                self.attr_d_op = tf.compat.v1.train.AdamOptimizer(
                    self.attr_d_lr, self.attr_d_beta1
                ).minimize(
                    self.attr_d_loss, var_list=self.attr_discriminator.trainable_vars
                )

        if self.sn_mode == "SN":
            self.spectral_norm_update_ops = tf.compat.v1.get_collection(self.SN_OP)
        elif self.sn_mode is None:
            pass
        else:
            raise ValueError("Unknown SN mode!")
    ...
    def build_summary(self):
        self.g_summary = []
        self.g_summary.append(tf.compat.v1.summary.scalar("loss/g/d", self.g_loss_d))
        if self.attr_discriminator is not None:
            self.g_summary.append(
                tf.compat.v1.summary.scalar("loss/g/attr_d", self.g_loss_attr_d)
            )
        self.g_summary.append(tf.compat.v1.summary.scalar("loss/g", self.g_loss))
        self.g_summary = tf.compat.v1.summary.merge(self.g_summary)

        self.d_summary = []
        self.d_summary.append(
            tf.compat.v1.summary.scalar("loss/d/fake", self.d_loss_fake)
        )
        self.d_summary.append(
            tf.compat.v1.summary.scalar("loss/d/real", self.d_loss_real)
        )
        self.d_summary.append(tf.compat.v1.summary.scalar("loss/d/gp", self.d_loss_gp))
        self.d_summary.append(tf.compat.v1.summary.scalar("loss/d", self.d_loss))
        self.d_summary.append(
            tf.compat.v1.summary.scalar("d/fake", tf.reduce_mean(self.d_fake_train_tf))
        )
        self.d_summary.append(
            tf.compat.v1.summary.scalar("d/real", tf.reduce_mean(self.d_real_train_tf))
        )
                ...
                for j in range(len(self.d_sigmas[i])):
                    self.d_summary.append(
                        tf.compat.v1.summary.scalar(
                            "d/sigma{}_{}".format(i, j), self.d_sigmas[i][j]
                        )
                    )
            else:
                self.d_summary.append(
                    tf.compat.v1.summary.scalar("d/sigma{}".format(i), self.d_sigmas[i])
                )
        self.d_summary = tf.compat.v1.summary.merge(self.d_summary)

        if self.attr_discriminator is not None:
            self.attr_d_summary = []
            self.attr_d_summary.append(
                tf.compat.v1.summary.scalar("loss/attr_d/fake", self.attr_d_loss_fake)
            )
            self.attr_d_summary.append(
                tf.compat.v1.summary.scalar("loss/attr_d/real", self.attr_d_loss_real)
            )
            self.attr_d_summary.append(
                tf.compat.v1.summary.scalar("loss/attr_d/gp", self.attr_d_loss_gp)
            )
            self.attr_d_summary.append(
                tf.compat.v1.summary.scalar("loss/attr_d", self.attr_d_loss)
            )
            self.attr_d_summary.append(
                tf.compat.v1.summary.scalar(
            ...
            for i in range(len(self.attr_d_sigmas)):
                if isinstance(self.attr_d_sigmas[i], list):
                    for j in range(len(self.attr_d_sigmas[i])):
                        self.attr_d_summary.append(
                            tf.compat.v1.summary.scalar(
                                "attr_d/sigma{}_{}".format(i, j),
                                self.attr_d_sigmas[i][j],
                            )
                        )
                else:
                    self.attr_d_summary.append(
                        tf.compat.v1.summary.scalar(
                            "attr_d/sigma{}".format(i), self.attr_d_sigmas[i]
                        )
                    )
    ...
    def load_pretrain(self, pretrain_dir):
        ckpt = tf.train.get_checkpoint_state(pretrain_dir)
        ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
        self.saver.restore(self.sess, os.path.join(pretrain_dir, ckpt_name))
    ...
    def discriminate_from(self, real_features, real_attributes):
        ...
        for i in range(round_):
            ...
            for j in range(self.num_packing):
                ...
                feed_dict[self.real_feature_pl_l[j]] = batch_data_feature
                feed_dict[self.real_attribute_pl_l[j]] = batch_data_attribute
            sub_results = self.sess.run(self.d_real_test_tf, feed_dict=feed_dict)
            ...
            if given_attribute is None:
                if feature_input_data.ndim == 2:
                    (
                        ...
                        feed_dict={
                            self.g_real_attribute_input_noise_test_pl: real_attribute_input_noise[
                                i * self.batch_size : (i + 1) * self.batch_size
                            ],
                            self.g_addi_attribute_input_noise_test_pl: addi_attribute_input_noise[
                                i * self.batch_size : (i + 1) * self.batch_size
                            ],
                            self.g_feature_input_noise_test_pl: feature_input_noise[
                                i * self.batch_size : (i + 1) * self.batch_size
                            ],
                            self.g_feature_input_data_test_free_pl: feature_input_data[
                                i * self.batch_size : (i + 1) * self.batch_size
                            ],
                        },
                    )
                    ...
                    (
                        ...
                        feed_dict={
                            ...
                            self.g_feature_input_data_test_teacher_pl: feature_input_data[
                                i * self.batch_size : (i + 1) * self.batch_size
                            ],
                        },
                    )
                ...
                (
                    ...
                    feed_dict={
                        ...
                        self.given_attribute_attribute_pl: given_attribute[
                            i * self.batch_size : (i + 1) * self.batch_size
                        ],
                    },
                )
    ...
    def gen_attribute_input_noise(self, num_sample):
        # print("Attribute noise type:", self.generator.attr_noise_type)

        if self.generator.attr_noise_type == "normal":
            return np.random.normal(size=[num_sample, self.attribute_latent_dim])
        elif self.generator.attr_noise_type == "uniform":
            return np.random.uniform(size=[num_sample, self.attribute_latent_dim])
        else:
            raise ValueError("Unknown attribute noise type!")

    def gen_feature_input_noise(self, num_sample, length):
        # print("Feature noise type:", self.generator.feature_noise_type)

        if self.generator.feature_noise_type == "normal":
            return np.random.normal(size=[num_sample, length, self.feature_latent_dim])
        elif self.generator.feature_noise_type == "uniform":
            return np.random.uniform(size=[num_sample, length, self.feature_latent_dim])
        else:
            raise ValueError("Unknown feature noise type!")

    def gen_feature_input_data_free(self, num_sample):
        return np.zeros(
            [num_sample, self.sample_len * self.sample_feature_dim], dtype=np.float32
        )

    def gen_feature_input_data_teacher(self, num_sample):
        id_ = np.random.choice(self.data_feature.shape[0], num_sample, replace=False)
        data_feature_ori = self.data_feature[id_, :, :]
        data_feature = np.reshape(
            data_feature_ori,
            [num_sample, self.sample_time, self.sample_len * self.sample_feature_dim],
        )
        ...
        ground_truth_length = np.sum(self.data_gen_flag[id_, :, :], axis=(1, 2))
    ...
    def visualize(self, epoch_id, batch_id, global_id):
        def sub1(
            ...
            file_path = os.path.join(
                self.sample_dir,
                "epoch_id-{},batch_id-{},global_id-{},type-{},samples.npz".format(
                    epoch_id, batch_id, global_id, type_
                ),
            )
            np.savez(
                file_path,
            ...
            file_path = os.path.join(
                self.sample_dir,
                "epoch_id-{},batch_id-{},global_id-{},type-{},feature".format(
                    epoch_id, batch_id, global_id, type_
                ),
            )
            if ground_truth_features is None:
                draw_feature(features, lengths, self.data_feature_outputs, file_path)
            else:
                draw_feature(
            ...
            file_path = os.path.join(
                self.sample_dir,
                "epoch_id-{},batch_id-{},global_id-{},type-{},attribute".format(
                    epoch_id, batch_id, global_id, type_
                ),
            )
            if ground_truth_features is None:
                draw_attribute(attributes, self.data_attribute_outputs, file_path)
            else:
                draw_attribute(
                    np.concatenate([attributes, ground_truth_attributes], axis=0),
                    self.data_attribute_outputs,
                    file_path,
                )

        real_attribute_input_noise = self.gen_attribute_input_noise(self.vis_num_sample)
        addi_attribute_input_noise = self.gen_attribute_input_noise(self.vis_num_sample)
        feature_input_noise = self.gen_feature_input_noise(
            self.vis_num_sample, self.sample_time
        )

        feature_input_data = self.gen_feature_input_data_free(self.vis_num_sample)
    ...
    def train(self, feature_network_checkpoint_path=None, restore=False):
        # This function prevents data loader (from another distributed process) having stdout.
        ...
        if restore is True:
            if self.pretrain_dir is None:
                raise ValueError("Fine tuning must need pretrained models!")
            # restore_global_id = self.load()
            # print("Loaded from global_id {}".format(restore_global_id))
            restore_global_id = self.load_pretrain(self.pretrain_dir)
            print("finised loading pretrained model ...")
            print("restore_global_id:", restore_global_id)
        ...
        if self.dp_noise_multiplier is not None and self.dp_l2_norm_clip is not None:
            ...
            # DEPRECATED: please use src/util.py/compute_dg_sgd_privacy_sum()
        ...
        for batch_id in tqdm(range(self.iteration), file=TqdmToLogger()):
            ...
            if global_id > restore_global_id:
                if (
                    ...
                    or batch_id == self.iteration - 1
                ):
                    with open(self.time_path, "a") as f:
                        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                        # f.write("iteration {}/{} starts: {}\n".format(batch_id, self.iteration, time))
                        f.write(
                            "iteration {}/{} starts: {}".format(
                                batch_id, self.iteration, time
                            )
                        )
            ...
            for i in range(self.num_packing):
                ...
                (
                    ...
                    batch_data_feature,
                    batch_data_gen_flag,
                ) = self.dataset.sample_batch(self.batch_size)

                batch_real_attribute_input_noise = self.gen_attribute_input_noise(
                    self.batch_size
                )
                batch_addi_attribute_input_noise = self.gen_attribute_input_noise(
                    self.batch_size
                )
                batch_feature_input_noise = self.gen_feature_input_noise(
                    self.batch_size, self.sample_time
                )
                batch_feature_input_data = self.gen_feature_input_data_free(
                    self.batch_size
                )

                feed_dict[self.real_feature_pl_l[i]] = batch_data_feature
                feed_dict[self.real_attribute_pl_l[i]] = batch_data_attribute
                feed_dict[
                    self.g_real_attribute_input_noise_train_pl_l[i]
            ...
            if global_id > restore_global_id:
                ...
                if self.combined_disc == True:
                    summary_result, _ = self.sess.run(
                        [self.d_summary, self.d_op], feed_dict=feed_dict
                    )
                    self.summary_writer.add_summary(summary_result, global_id)

                if self.attr_discriminator is not None:
                    summary_result, _ = self.sess.run(
                        [self.attr_d_summary, self.attr_d_op], feed_dict=feed_dict
                    )
                    self.summary_writer.add_summary(summary_result, global_id)
                ...
                summary_result, _ = self.sess.run(
                    [self.g_summary, self.g_op], feed_dict=feed_dict
                )
                self.summary_writer.add_summary(summary_result, global_id)
            ...
            if global_id - 1 > restore_global_id:
                if (
                    ...
                    with open(self.time_path, "a") as f:
                        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                        # f.write("iteration {}/{} ends: {}\n".format(batch_id, self.iteration, time))
                        f.write(
                            "iteration {}/{} ends: {}".format(
                                batch_id, self.iteration, time
                            )
                        )

                if (batch_id + 1) % self.extra_checkpoint_freq == 0:
                    saver = tf.compat.v1.train.Saver()
                    checkpoint_dir = os.path.join(
                        self.checkpoint_dir, "iteration_id-{}".format(batch_id)
                    )
