...
from .util import (
    denormalize,
    _merge_syn_df,
    _recalulate_config_ids_in_each_config_group,
)
...
class NetsharePrePostProcessor(PrePostProcessor):
    def _pre_process(self, input_folder, output_folder, log_folder):
        ...
        if self._config["dataset_type"] == "pcap":
            if input_folder.endswith(".csv"):
                shutil.copyfile(
                    os.path.join(input_folder), os.path.join(output_folder, "raw.csv")
                )
                ...
                csv_file = os.path.join(
                    output_folder,
                    os.path.splitext(os.path.basename(input_folder))[0] + ".csv",
                )
                pcap2csv_func(
        ...
        if os.path.exists(
            os.path.join(
                ...
                "word2vec_vecSize_{}.model".format(self._config["word2vec_vecSize"]),
            )
        ):
        ...
        if self._config["encode_IP"] == "bit":
            fields["srcip"] = BitField(name="srcip", num_bits=32)

            fields["dstip"] = BitField(name="dstip", num_bits=32)
        ...
        if "multichunk_dep" in self._config["split_name"]:
            fields["startFromThisChunk"] = DiscreteField(
                name="startFromThisChunk", choices=[0.0, 1.0]
            )
        ...
        if self._config["timestamp"] == "interarrival":
            fields["flow_start"] = ContinuousField(
                name="flow_start", norm_option=NORM_OPTION
            )
            fields["interarrival_within_flow"] = ContinuousField(
                name="interarrival_within_flow", norm_option=NORM_OPTION
            )
        elif self._config["timestamp"] == "raw":
            if self._config["dataset_type"] == "pcap":
                fields["time"] = ContinuousField(name="time", norm_option=NORM_OPTION)
            elif self._config["dataset_type"] == "netflow":
                fields["ts"] = ContinuousField(name="ts", norm_option=NORM_OPTION)
        else:
            raise ValueError(
                "Timestamp encoding can be only \
                `interarrival` or 'raw"
            )
        ...
        if self._config["dataset_type"] == "pcap":
            fields["pkt_len"] = ContinuousField(
                name="pkt_len",
                min_x=(float(df["pkt_len"].min()) if not self._config["dp"] else 20.0),
                max_x=(
                    float(df["pkt_len"].max()) if not self._config["dp"] else 1500.0
                ),
            ...
            if self._config["full_IP_header"]:
                fields["tos"] = ContinuousField(
                    name="tos", min_x=0.0, max_x=255.0, norm_option=NORM_OPTION
                )
                fields["id"] = ContinuousField(
                    name="id", min_x=0.0, max_x=65535.0, norm_option=NORM_OPTION
                )
                fields["flag"] = DiscreteField(name="flag", choices=[0, 1, 2])
            ...
            for field in ["label", "type"]:
                if field in df.columns:
                    fields[field] = DiscreteField(
                        name=field, choices=list(set(df[field]))
                    )
        ...
        for chunk_id, df_chunk in enumerate(df_chunks):
            print("Chunk_id: {}, # of pkts/records: {}".format(chunk_id, len(df_chunk)))
        ...
        flowkeys_chunkidx_file = os.path.join(output_folder, "flowkeys_idxlist.json")
        ...
        print(
            "# of flows cross chunk (of total flows): {} ({}%)".format(
        ...
        for chunk_id, df_chunk in tqdm(enumerate(df_chunks)):
            (
                ...
                data_feature_output,
                fields_per_epoch,
            ) = objs_output[chunk_id]
            # TODO: add pre_process multiple configurations
            data_out_dir = os.path.join(output_folder, f"chunkid-{chunk_id}")
            os.makedirs(data_out_dir, exist_ok=True)

            df_chunk.to_csv(os.path.join(data_out_dir, "raw.csv"), index=False)
            ...
            with open(
                os.path.join(data_out_dir, "data_feature_output.pkl"), "wb"
            ) as fout:
                pickle.dump(data_feature_output, fout)
            with open(
                os.path.join(data_out_dir, "data_attribute_output.pkl"), "wb"
            ) as fout:
    ...
    def _post_process(self, input_folder, output_folder, log_folder):
        ...
        print("POSTPROCESSOR......")
        print(input_folder)
        paths = [os.path.join(input_folder, p) for p in os.listdir(input_folder)]
        ...
        for path in paths:
            ...
            for d in data_files:
                ...
                syn_df = denormalize(attributes, features, gen_flags, config)
                chunk_id, iteration_id = re.search(
                    r"chunk_id-(\d+)_iteration_id-(\d+).npz", d
                ).groups()
                print(syn_df.shape)

                save_path = os.path.join(syn_path, "chunk_id-{}".format(chunk_id))
                os.makedirs(save_path, exist_ok=True)
                syn_df.to_csv(
                    os.path.join(
                        save_path, "syn_df_iteration_id-{}.csv".format(iteration_id)
                    ),
                    index=False,
                )

        # Step 2: pick the best among hyperparameters/tranining snapshots
        config_group_list = _recalulate_config_ids_in_each_config_group(configs)
        work_folder = os.path.dirname(input_folder)
        _merge_syn_df(
            configs=configs,
            config_group_list=config_group_list,
            big_raw_df=pd.read_csv(
                os.path.join(work_folder, "pre_processed_data", "raw.csv")
            ),
