...
from netshare.utils import BitField, ContinuousField, DiscreteField, Normalization
...
def countList2cdf(count_list):
    ...
    for x in count_list:
        if x not in dist_dict:
            dist_dict[x] = 0
        dist_dict[x] += 1
    dist_dict = {k: v for k, v in sorted(dist_dict.items(), key=lambda x: x[0])}  # type: ignore
    x = dist_dict.keys()
...
def df2chunks(
    big_raw_df, config_timestamp, split_type="fixed_size", n_chunks=10, eps=1e-5
):
...
def apply_per_field(original_df, config_fields, field_instances, embed_model=None):
...
def split_per_chunk(
    ...
    if config["n_chunks"] > 1:
        ...
        for chunk_id in range(config["n_chunks"]):
            metadata_fields.append(
                DiscreteField(name="chunk_{}".format(chunk_id), choices=[0.0, 1.0])
            )
    ...
    if (
        config["timestamp"]["generation"]
        and config["timestamp"]["encoding"] == "interarrival"
    ):
