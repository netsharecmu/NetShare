...
def plot_cdf(
    ...
    if metric == "flow_size":
        x, cdf = vals2cdf(
            raw_df.groupby(["srcip", "dstip", "srcport", "dstport", "proto"])
            .size()
            .values
        )
    else:
    ...
    for method, syn_df in syn_df_dict.items():
        ...
        if metric == "flow_size":
            x, cdf = vals2cdf(
                syn_df.groupby(["srcip", "dstip", "srcport", "dstport", "proto"])
                .size()
                .values
            )
        elif metric == "td":
        ...
        if method == "NetShare":
            ...
            plt.plot(
                x, cdf, label=label_method, color=CB_color_cycle[idx], linewidth=1.5
            )
...
def plot_bar(
    ...
    fig.text(0.0, 0.5, ylabel, ha="center", va="center", fontsize=20, rotation=90)
...
def compute_metrics_netflow_v2(raw_df, syn_df):
    ...
    for metric in ["ts", "td", "pkt", "byt"]:
        metrics_dict[metric] = jsd(
            list(raw_df[metric]), list(syn_df[metric]), type="continuous"
        )
        ...
        for method in ["CTGAN-A", "CTGAN-B", "STAN", "E-WGAN-GP", "NetShare"]:
            ...
            for synid in range(3):
                syn_df_file = os.path.join(
                    data_baseDir, dataset, method, "syn{}.csv".format(synid + 1)
                )
                ...
                for metric, val in metrics_dict.items():
                    ...
                    meta_metric_dict_netflow[dataset][method][metric].append(val)
...
def run_netflow_qualitative_plots():
    for dataset in ["ugr16", "cidds", "ton"]:
        ...
        for method in ["NetShare"]:
            ...
            # )
            syn_df = pd.read_csv(
                os.path.join(data_baseDir, dataset, method, "syn1.csv")
            )
        ...
        for metric, xlabel in {
            ...
            plot_HH(
                ...
                plot_loc=os.path.join(
                    result_folder_per_dataset, "hh_{}.pdf".format(metric)
                ),
                metric=metric,
                x_logscale=True,
                y_logscale=True,
        ...
        for metric, xlabel in {
            ...
            plot_bar(
                ...
                plot_loc=os.path.join(
                    result_folder_per_dataset, "bar_{}.pdf".format(metric)
                ),
                metric=metric,
                x_logscale=False,
                y_logscale=False,
        ...
        for metric, xlabel in {
            ...
            plot_cdf(
                ...
                ylabel="CDF",
                plot_loc=os.path.join(
                    result_folder_per_dataset, "cdf_{}.pdf".format(metric)
                ),
                metric=metric,
...
def run_netflow_qualitative_plots_dashboard(raw_data_path, syn_data_path, plot_dir):
    ...
    for metric, xlabel in {
        ...
        plot_HH(
            ...
            xlabel=xlabel,
            ylabel="Relative frequency (log-scale)",
            plot_loc=os.path.join(plot_dir, "hh_{}.png".format(metric)),
            metric=metric,
            x_logscale=True,
            y_logscale=True,
        )
    ...
    for metric, xlabel in {
        ...
        plot_bar(
            ...
            xlabel=xlabel,
            ylabel="Relative frequency",
            plot_loc=os.path.join(plot_dir, "bar_{}.png".format(metric)),
            metric=metric,
            x_logscale=False,
            y_logscale=False,
            data_type="netflow",
    ...
    for metric, xlabel in {
        ...
        plot_cdf(
            ...
            xlabel=xlabel,
            ylabel="CDF",
            plot_loc=os.path.join(plot_dir, "cdf_{}.png".format(metric)),
            metric=metric,
            x_logscale=(metric != "td"),
        )
...
def run_pcap_qualitative_plots():
    ...
    for dataset in ["caida", "dc", "ca"]:
        ...
        for method in ["NetShare"]:
            ...
            syn_df = pd.read_csv(
                os.path.join(data_baseDir, dataset, method, "syn1.csv".format(method))
            )
            syn_df_dict[method] = syn_df
            # print(syn_df.head())
...
def run_caida_flowsize():
    for dataset in ["caida"]:
        ...
        for metric, xlabel in {"flow_size": "Flow size (# of packets perflow)"}.items():
            plot_cdf(
                raw_df=raw_df,
                syn_df_dict=syn_df_dict,
                xlabel=xlabel,
