...
def change_order(path, figure_stored_relative_folder_for_vis):
    ...
    static_path_files = [
        os.path.join(figure_stored_relative_folder_for_vis, x) for x in files
    ]
    ...
    for i in range(10):
        ...
        for _, x in enumerate(static_path_files):
            if suffix in x:
                sorted_static_path_files.append({"path": x, "caption": caption_list[i]})
                break
...
class Dashboard:
    def __init__(self, plot_folder, figure_stored_relative_folder_for_vis):
        ...
        def dashboard():
            ...
            return render_template("index.html", static_paths=sorted_static_path_files)
